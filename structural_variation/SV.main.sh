...
for line in $(seq 1 $sample_num); do
  ...
  ## only retain ITX and CTX, at least three reads to support this ITX and CTX
  ...
  grep -E 'SVTYPE=CTX' $outdir/Manta_BreakDancer/$id/breakdancer.filt.vcf > $outdir/Manta_BreakDancer/$id/CTX.vcf
